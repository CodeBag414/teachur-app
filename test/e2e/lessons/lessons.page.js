...
var ObjectivesPage = function() {
  var pagesMap = {
    single: config.baseUrl + '#/lessons/:id',
    create: config.baseUrl + '#/lessons/create',
    edit: config.baseUrl + '#/lessons/:id/edit',
  };
  ...
  this.create = function() {
    element(by.model('lesson.name')).sendKeys(getRandomString(5));
    return element(by.id('save-btn')).click();
  };

  this.update = function() {
    element(by.model('lesson.name')).clear().sendKeys('updated item');
    return element(by.id('save-btn')).click();
  };
};
